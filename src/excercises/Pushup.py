...
class Pushup(Exercise):
    ...
    def exercise(self, source):
        ...
        scount = 0
        inaccurateCount = prevInaccurateCount =0
        clearUp = clearDown = wentUp = wentDown = done = False
        while True:
            ...
            try:
                ...
                if 12 in idx_to_coordinates and 28 in idx_to_coordinates and 16 in idx_to_coordinates:  # right side of body
            ...
            try:
                ...
                if 12 in idx_to_coordinates and 14 in idx_to_coordinates and 16 in idx_to_coordinates:
                    eang1 = ang((idx_to_coordinates[12], idx_to_coordinates[14]),
                        (idx_to_coordinates[14], idx_to_coordinates[16]))
                elif 11 in idx_to_coordinates and 13 in idx_to_coordinates and 15 in idx_to_coordinates:
                    eang1 = ang((idx_to_coordinates[11], idx_to_coordinates[13]),
                                (idx_to_coordinates[13], idx_to_coordinates[15]))
                ...
                print(f'An error in counting : {e} ')
                print('pushup count:', scount)
                pass

            try:
                if 12 in idx_to_coordinates and 14 in idx_to_coordinates and 16 in idx_to_coordinates:
                    eang2 = ang((idx_to_coordinates[12], idx_to_coordinates[14]),
                        (idx_to_coordinates[14], idx_to_coordinates[16]))
                elif 11 in idx_to_coordinates and 13 in idx_to_coordinates and 15 in idx_to_coordinates:
                    eang2 = ang((idx_to_coordinates[11], idx_to_coordinates[13]),
                                (idx_to_coordinates[13], idx_to_coordinates[15]))

                # Inaccurate pushup count
                # if eang2 <= 70:
                #     wentDown = True
                #     clearUp = False
                # if eang2 > 160 :
                #     clearUp = True
                #     wentDown = False
                # if eang2 > 70 and not clearUp:
                #     wentUp = True
                # if eang2 <=160  and not wentDown:
                #     clearDown = False
                # if eang2 < 70 and wentDown and wentUp and not clearUp:
                #     inaccurateCount += 1
                #     wentDown = False
                #     wentUp = False
                # if eang2 > 160 and clearUp and clearDown and not wentDown:
                #     inaccurateCount += 1
                #     clearUp = False
                #     clearDown = False
                # if inaccurateCount != prevInaccurateCount:
                #     prevInaccurateCount = inaccurateCount
                #     print("Inaccurate count",inaccurateCount)

                # accuracy = (scount/(scount+inaccurateCount))*100
            ...
            if cv2.waitKey(5) & 0xFF == 27:
                if accuracy >= 80:
                    print('Pushup Exercise Successful')
                    print('pushup count:', scount)
                    print('Accuracy:',accuracy)
                else:
                    print('''Your posture is a little incorrect , try to improve it''')
                    print('pushup count:', scount)
                    print('Accuracy:',accuracy)
                print("Exiting...")
                break
            if threaded_camera.total_frames<=0:
                print('Valid pushup Count - ',scount)
                break
        pose.close()
