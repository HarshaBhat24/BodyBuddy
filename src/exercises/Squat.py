    ...
    def exercise(self, source):
        ...
        while True:
            ...
            try:
                ...
                pass
            try:
                # elbow - wrist - horizontal ground [for count]
                if 25 in idx_to_coordinates and 31 in idx_to_coordinates:  # left side of body
                    cv2.line(image, (idx_to_coordinates[25]), (idx_to_coordinates[31]), thickness=4,
                             color=(255, 0, 255))
                    cv2.line(image, (idx_to_coordinates[31]),
                             (idx_to_coordinates[31][0] + 80, idx_to_coordinates[31][1]),
                             thickness=4, color=(255, 0, 255))
                    l1 = np.linspace(idx_to_coordinates[25], idx_to_coordinates[31], 100)
                    temp = (idx_to_coordinates[31][0] + 80, idx_to_coordinates[31][1])
                    l2 = np.linspace(idx_to_coordinates[31], temp, 100)
                    ang1 = ang((idx_to_coordinates[25], idx_to_coordinates[31]),
                               (idx_to_coordinates[31], temp))
                    cv2.putText(image, "   " + str(round(ang1, 2)), (idx_to_coordinates[31]),
                                fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                                fontScale=0.6, color=(0, 255, 0), thickness=2)
                    center, radius, start_angle, end_angle = convert_arc(l1[80], l2[20], sagitta=15)
                    axes = (radius, radius)
                    ...
                    cv2.line(image, (idx_to_coordinates[26]), (idx_to_coordinates[32]), thickness=4,
                             color=(255, 0, 255))
                    cv2.line(image, (idx_to_coordinates[32]),
                             (idx_to_coordinates[32][0] + 80, idx_to_coordinates[32][1]),
                             thickness=4, color=(255, 0, 255))
                    l1 = np.linspace(idx_to_coordinates[25], idx_to_coordinates[32], 100)
                    temp = (idx_to_coordinates[32][0] + 80, idx_to_coordinates[32][1])
                    l2 = np.linspace(idx_to_coordinates[32], temp, 100)
                    ang1 = ang((idx_to_coordinates[25], idx_to_coordinates[32]),
                               (idx_to_coordinates[32], temp))
                    cv2.putText(image, "   " + str(round(ang1, 2)), (idx_to_coordinates[32]),
                                fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                                fontScale=0.6, color=(0, 255, 0), thickness=2)
                    center, radius, start_angle, end_angle = convert_arc(l1[80], l2[20], sagitta=15)
                    axes = (radius, radius)
            ...
            if cv2.waitKey(5) & 0xFF == 27:
                print('Valid squat Count - ', scount)
                break
            if threaded_camera.total_frames<=0:
                print('Valid squat Count - ',scount)
                break            
        pose.close()
